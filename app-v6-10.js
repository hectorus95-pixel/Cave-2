  ...
  $('#moveSlotGrid').innerHTML=slots.length
    ? slots.map(x=>`
      <button type="button" class="move-slot-place"
        ...
        <b>C${Number(x.casier)} · L${Number(x.ligne)} · P${Number(x.position)}</b>
        <span>Déplacer ici</span>
      </button>
    `).join('')
    : '<div class="move-slot-empty">Aucun emplacement libre dans ce casier.</div>';
...
$('#moveSlotGrid').addEventListener('click',e=>{
  const b=e.target.closest('[data-move-slot-line]');
  if(!b || !moveSource?.items?.length) return;
...
function makeBackupPayload(){
  return {
    version:61000,
    app:'ma-cave-configurable-v6.10',
...
$('#export').addEventListener('click',async ()=>{
  ...
  const filename='sauvegarde-ma-cave-configurable-v6-10.json';
